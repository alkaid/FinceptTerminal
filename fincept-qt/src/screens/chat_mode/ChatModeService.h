...
class ChatModeService : public QObject {
  ...
  public:
    ...
    void save_message(const QString& session_uuid, const QString& role, const QString& content, MessageCallback cb,
                      const QString& provider = {}, const QString& model = {}, int tokens_used = 0,
    ...
    void export_sessions(const QStringList& uuids, std::function<void(bool ok, QJsonArray data, QString error)> cb);
    ...
    void agent_chat(const QString& query, const QString& session_id, StreamMode mode, AgentChatCallback cb,
    ...
    void save_memory(const QString& key, const QString& value, const QString& memory_type, VoidCallback cb);
    ...
    void create_schedule(const QString& query, const QString& cron_expression, const QString& session_id,
                         ScheduleCallback cb);
    ...
    using ActivityCallback =
        std::function<void(bool ok, QString task_id, QVector<TaskActivity> events, int count, QString error)>;
    ...
    void get_task_activity(const QString& task_id, ActivityCallback cb, int after_id = 0, int limit = 500);
    ...
    // ── MCP servers ───────────────────────────────────────────────────────────
    using McpServersCallback = std::function<void(bool ok, QVector<McpServer> servers, int total_tools, QString error)>;
    ...
    void create_monitor(const QString& name, const QString& source_type, const QJsonObject& source_config,
                        const QJsonObject& trigger_config, const QString& analysis_query, int check_interval_seconds,
    ...
    void register_terminal_tools(const QJsonArray& tools, const QString& version, int tool_count, RegisterCallback cb);
    ...
    QNetworkReply* stream_message(const QString& message, const QString& session_id, StreamMode mode = StreamMode::Lite,
                                  const QString& source = {}, bool auto_approve = false, int profile_id = 1);

    /// Abort any active SSE stream.
    void abort_stream();

    /// Low-level GET for callers that need raw document access (e.g. messages array).
    void get(const QString& path, std::function<void(bool ok, QJsonDocument doc, QString error)> cb);
  ...
  private:
    ...
    void put(const QString& path, const QJsonObject& body,
             std::function<void(bool ok, QJsonDocument doc, QString error)> cb);
    void del(const QString& path, std::function<void(bool ok, QJsonDocument doc, QString error)> cb);
    void del_with_body(const QString& path, const QJsonObject& body,
                       std::function<void(bool ok, QJsonDocument doc, QString error)> cb);

    void handle_reply(QNetworkReply* reply, std::function<void(bool ok, QJsonDocument doc, QString error)> cb);
